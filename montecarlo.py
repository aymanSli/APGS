...
class MonteCarlo:
    ...
    def calculate_deltas(self, past_matrix, current_date, seed=None):
        ...
        for asset_index in range(num_assets):
            ...
            shifted_up = self.simulation.shift_path(
                paths, asset_index, 1 + self.fd_steps, current_row
            )
            ...
            for sim in range(self.num_samples):
                ...
                payoff_up = self.product.calculate_total_payoff(path_up)['total_payoff']
                up_payoffs.append(payoff_up)
            
            # Create shifted paths for this asset (down)
            shifted_down = self.simulation.shift_path(
                paths, asset_index, 1 - self.fd_steps, current_row
            )
            ...
            for sim in range(self.num_samples):
                ...
                payoff_down = self.product.calculate_total_payoff(path_down)['total_payoff']
